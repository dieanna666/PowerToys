...
namespace Microsoft.PowerToys.Run.Plugin.WindowsTerminal.Helpers
{
    public class TerminalQuery : ITerminalQuery
    {
        ...
        public IEnumerable<TerminalProfile> GetTerminals()
        {
            ...
            foreach (var terminal in _terminals)
            {
                ...
                profilesElement.TryGetProperty("list", out JsonElement list);
                ...
                {
                    profiles.Add(ParseProfile(terminal, p));
                }
            }
        ...
        private static TerminalProfile ParseProfile(TerminalPackage terminal, JsonElement element)
        {
            ...
            element.TryGetProperty("guid", out JsonElement guidElement);
            var guid = guidElement.ValueKind == JsonValueKind.String ? Guid.Parse(guidElement.GetString()) : null as Guid?;

            element.TryGetProperty("icon", out JsonElement iconElement);
